...
(defun paw-focus-find-current-word(&optional word)
  (interactive)
  (let* ((word (or word (thing-at-point 'word t) ))
         (lang (paw-check-language word)))
    (cond ((string= lang "en")
           (paw-ecdict-command word 'paw-focus-find-current-word-sentinel-english "WORD"))
          ((string= lang "ja")
...
(defun paw-focus-find-current-word-sentinel-english (proc _event)
  "Handles the english process termination event."
  (when (eq (process-status proc) 'exit)
    (let* ((json-object-type 'plist)
           (json-array-type 'list)
