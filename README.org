...
- python dependencies (install paw command line by ~pip install emacs-paw~ or one by one manually):
  ...
  - mecab-python3
  - unidic-lite
  - uvicorn (optional, for enhanced server performance)
...
6. paw-server is optional, you don't need to run it, and still able to use the org-protocol feature. If you installed paw command line, you can run paw-server in several ways:

   **Method 1: Emacs Integration (Original)**
        #+begin_src sh
   ...
   **Method 3: Direct Python Execution**
   ...
   The standalone mode offers enhanced stability, better error handling, environment variable support, and uvicorn support for better performance.
