...
Set ~paw-detect-language-p~ to t, and run ~pip install pycld2~, paw will use pycld2 (maybe use others tools in the future) to detect the language for accurate tts pronunciation and translation.

Setup ~paw-python-program~ if necessary, if the pip modules is installed with different python version 
