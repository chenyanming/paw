...
4. Annotations will be highlighted via overlay. Under the annotation, you can do
   ...
   dedicated for this annotation, ~C-c C-i~ to insert annotation link as reflink, finally save by ~C-c C-c~, changes will be saved in
   the database respectively.
4. Under the annotation, run ~paw-change-annotation-note-type~ (~c~) or ~paw-change-note_type~ (~C~) to change the type of overlay. paw-change-annotation-note-type will remember the last overlay. 
