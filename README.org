...
For example, the following snippet show the gptel result to paw buffer by using ~paw-view-note~ and ~paw-new-entry~ functions, and further interact with the result in ~*paw-view-note*~ buffer.
#+begin_src emacs-lisp
      ...
      (lambda (response info)
        (if (not response)
          ...
          (with-current-buffer (get-buffer-create "*gptel-quick*")
            (let ((paw-say-word-p nil))
              (paw-view-note (paw-new-entry gptel-last-prompt
                                            :origin_type "gptel"
                                            :serverp 3
                                            :lang (paw-check-language gptel-last-prompt)
                                            :context response)
