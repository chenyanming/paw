...
PS: Start from 1.1.3, paw has its own sdcv configurations for better future development (for Chinese/Jieba: we are still using need [[https://github.com/manateelazycat/sdcv][sdcv]] though), use the following configurations to support both [[https://github.com/manateelazycat/sdcv][sdcv]] and paw-sdcv:
#+begin_src emacs-lisp
(if (featurep 'sdcv)
      (progn
